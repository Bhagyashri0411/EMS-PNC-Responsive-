/**
* @preserve CanvasJS HTML5 & JavaScript Charts - v3.4.9 GA - https://canvasjs.com/ 
...
(function () {var isDebugMode = false;
var global = {};
...
var defaultOptions = {
	...
	Axis: {
		...
		includeZero: false, //Applies only for axisY. Ignored in axisX.
...
//Check if passive option is supported or not
...
function addEvent(obj, eventType, fn, store, useCapture) {
	store = store || [];
	useCapture = !isNullOrUndefined(useCapture) ? useCapture : (isPassiveSupported ? { passive: false, capture: false } : false);
...
var getAutoFontSize = function (standardSize, width, height) {
	...
	return Math.max(this.theme === "theme4" ? 0 : (size >= 300 ? 12 : 11), Math.round(size * fontSizeScaleFactor));
}
	...
	return function (dt, formatString, cultureInfo) {
		...
		result = formatString.replace(reg, function (key) {
			...
			switch (key) {
				...
				case "f":
					return pad(String(milliseconds), 3).slice(0, 1);
				case "ff":
					return pad(String(milliseconds), 3).slice(0, 2);
				case "fff":
					return pad(String(milliseconds), 3).slice(0, 3);
...
function addExportMenu(chart) {
	...
	if (chart._menuButton) {
		if (chart.exportEnabled) {
			setCSSProperties(chart._menuButton, { backgroundColor: chart.toolbar.itemBackgroundColor, color: chart.toolbar.fontColor });
			show(chart._menuButton);
			chart._menuButton.title = chart._cultureInfo.menuText;
		}
		else
			hide(chart._menuButton);
	} else if (chart.exportEnabled && isCanvasSupported) {
		var preventMouseEvent = false;
		chart._menuButton = document.createElement("button");
		setButtonState(chart, chart._menuButton, "menu");
		chart._toolBar.appendChild(chart._menuButton);

		addEvent(chart._menuButton, "touchstart", function (e) {
			preventMouseEvent = true;
		}, chart.allDOMEventHandlers);

		addEvent(chart._menuButton, "click", function () {
			if (chart._dropdownMenu.style.display === "none") {

				if (chart._dropDownCloseTime && ((new Date()).getTime() - chart._dropDownCloseTime.getTime() <= 500))
					return;

				chart._dropdownMenu.style.display = "block";
				chart._menuButton.blur();
				chart._dropdownMenu.focus();
			}

		}, chart.allDOMEventHandlers, true);
		addEvent(chart._menuButton, "mousemove", function () {
			if (!preventMouseEvent) {
				setCSSProperties(chart._menuButton, { backgroundColor: chart.toolbar.itemBackgroundColorOnHover, color: chart.toolbar.fontColorOnHover })
				if (navigator.userAgent.search("MSIE") <= 0)
					setCSSProperties(chart._menuButton.childNodes[0], { WebkitFilter: "invert(100%)", filter: "invert(100%)" });
			}
		}, chart.allDOMEventHandlers, true);

		addEvent(chart._menuButton, "mouseout", function () {
			if (!preventMouseEvent) {
				setCSSProperties(chart._menuButton, { backgroundColor: chart.toolbar.itemBackgroundColor, color: chart.toolbar.fontColor })
				if (navigator.userAgent.search("MSIE") <= 0)
					setCSSProperties(chart._menuButton.childNodes[0], { WebkitFilter: "invert(0%)", filter: "invert(0%)" });
			}
		}, chart.allDOMEventHandlers, true);
	}

	if (chart.exportEnabled && chart._dropdownMenu) {
		setCSSProperties(chart._dropdownMenu, { backgroundColor: chart.toolbar.itemBackgroundColor, color: chart.toolbar.fontColor });
		var exportOptions = chart._dropdownMenu.childNodes;
		var exportCultureOptions = [chart._cultureInfo.printText, chart._cultureInfo.saveJPGText, chart._cultureInfo.savePNGText];
		for (var i = 0; i < exportOptions.length; i++) {
			setCSSProperties(exportOptions[i], { backgroundColor: chart.toolbar.itemBackgroundColor, color: chart.toolbar.fontColor });
			exportOptions[i].innerHTML = exportCultureOptions[i];
		}
	}
	else if (!chart._dropdownMenu && chart.exportEnabled && isCanvasSupported) {
		var preventMouseEvent = false;
		chart._dropdownMenu = document.createElement("div");
		chart._dropdownMenu.setAttribute("tabindex", -1);
		var shadowColor = chart.theme.indexOf("dark") !== -1 ? "black" : "#888888";
		chart._dropdownMenu.style.cssText = "position: absolute; z-index: 1; -webkit-user-select: none; -moz-user-select: none; -ms-user-select: none; user-select: none; cursor: pointer;right: 0px;top: 25px;min-width: 120px;outline: 0;font-size: 14px; font-family: Arial, Helvetica, sans-serif;padding: 5px 0px 5px 0px;text-align: left;line-height: 10px;background-color:" + chart.toolbar.itemBackgroundColor + ";box-shadow: 2px 2px 10px " + shadowColor;
		chart._dropdownMenu.style.display = "none";
		chart._toolBar.appendChild(chart._dropdownMenu);

		addEvent(chart._dropdownMenu, "blur", function () {
			hide(chart._dropdownMenu);

			chart._dropDownCloseTime = new Date();
		}, chart.allDOMEventHandlers, true);

		var exportOption = document.createElement("div");
		exportOption.style.cssText = "padding: 12px 8px 12px 8px"
		exportOption.innerHTML = chart._cultureInfo.printText;
		exportOption.style.backgroundColor = chart.toolbar.itemBackgroundColor;
		exportOption.style.color = chart.toolbar.fontColor;
		chart._dropdownMenu.appendChild(exportOption);

		addEvent(exportOption, "touchstart", function (e) {
			preventMouseEvent = true;
		}, chart.allDOMEventHandlers);

		addEvent(exportOption, "mousemove", function () {
			if (!preventMouseEvent) {
				this.style.backgroundColor = chart.toolbar.itemBackgroundColorOnHover;
				this.style.color = chart.toolbar.fontColorOnHover;
			}
		}, chart.allDOMEventHandlers, true);

		addEvent(exportOption, "mouseout", function () {
			if (!preventMouseEvent) {
				this.style.backgroundColor = chart.toolbar.itemBackgroundColor;
				this.style.color = chart.toolbar.fontColor;
			}
		}, chart.allDOMEventHandlers, true);
		...
		var exportOption = document.createElement("div");
		exportOption.style.cssText = "padding: 12px 8px 12px 8px"
		exportOption.innerHTML = chart._cultureInfo.saveJPGText;
		exportOption.style.backgroundColor = chart.toolbar.itemBackgroundColor;
		exportOption.style.color = chart.toolbar.fontColor;
		chart._dropdownMenu.appendChild(exportOption);
		...
		addEvent(exportOption, "click", function () {
			chart.exportChart({ format: "jpeg", fileName: chart.exportFileName });
			hide(chart._dropdownMenu);
		}, chart.allDOMEventHandlers, true);

		var exportOption = document.createElement("div");
		exportOption.style.cssText = "padding: 12px 8px 12px 8px"
		exportOption.innerHTML = chart._cultureInfo.savePNGText;
		exportOption.style.backgroundColor = chart.toolbar.itemBackgroundColor;
		exportOption.style.color = chart.toolbar.fontColor;
		chart._dropdownMenu.appendChild(exportOption);
		...
		addEvent(exportOption, "click", function () {
			chart.exportChart({ format: "png", fileName: chart.exportFileName });
			//exportCanvas(chart.canvas, "png", chart.exportFileName);
			hide(chart._dropdownMenu);
		}, chart.allDOMEventHandlers, true);
	}
}
...
function setCSSProperties(element, cssStyle) {
...
function setButtonState(chart, button, state) {
	if (!button.getAttribute("state")) {
		...
		setCSSProperties(button, { "WebkitUserSelect": "none", "MozUserSelect": "none", "msUserSelect": "none", "userSelect": "none" });
	}
	if (button.getAttribute("state") !== state) {

		button.setAttribute("state", state);
		button.setAttribute("type", 'button');
		setCSSProperties(button, { "padding": "5px 12px", "cursor": "pointer", "float": "left", "width": "40px", "height": "25px", "outline": "0px", "verticalAlign": "baseline", "lineHeight": "0" });
...
function CanvasJSObject(defaultsKey, themeOptionsKey, options, index, parent) {
	...
	if (this.theme && isNullOrUndefined(this.parent) && isNullOrUndefined(themeOptionsKey) && isNullOrUndefined(index)) {
		currentThemeOptions = !isNullOrUndefined(this.predefinedThemes[this.theme]) ? this.predefinedThemes[this.theme] : this.predefinedThemes["light1"];
	}
	else {
...
TextBlock.prototype.render = function (preserveContext) {
	...
	for (var i = 0; i < this._wrappedText.lines.length; i++) {
		...
		if (this.horizontalAlign === "right" || this.textAlign === "right")
			textLeft = this.width - line.width - this.rightPadding;
		else if (this.horizontalAlign === "left" || this.textAlign === "left")
			textLeft = this.leftPadding;
		else if (this.horizontalAlign === "center" || this.textAlign === "center")
...
Title.prototype.setLayout = function () {
	...
	if (!this.wrap)
		maxHeight = Math.min(maxHeight, this.fontSize * 1.5 + padding);
...
if (typeof module !== 'undefined' && typeof module.exports !== 'undefined') {
	...
	if (typeof define === 'function' && define.amd) {
		...
		if (window.CanvasJS && window.console)
			window.console.log("CanvasJS namespace already exists. If you are loading both chart and stockchart scripts, just load stockchart alone as it includes all chart features.");

		window.CanvasJS = window.CanvasJS ? window.CanvasJS : CanvasJS;
	}
}
//#endregion Public API


//This variable is used to keep local reference of Chart
var CanvasJSChart = CanvasJS.Chart = (function () {
	...
	function Chart(containerId, options) {
		...
		if (!isNullOrUndefined(arguments[2])) {
			var initParameters = arguments[2];

			this.parent = initParameters.parent;
			this.index = initParameters.index;
			this.predefinedThemes = initParameters.predefinedThemes;
			this.optionsName = initParameters.optionsName;
			this.stockChart = initParameters.stockChart;
			this.panel = containerId;
			this.isOptionsInArray = initParameters.isOptionsInArray;
		} else {
			this.predefinedThemes = themes;
			this.index = null;
			this.parent = null;
			this.optionsName = null;
		}

		this.theme = (!isNullOrUndefined(options.theme) && !isNullOrUndefined(this.predefinedThemes[options.theme])) ? options.theme : "light1";


		Chart.base.constructor.call(this, "Chart", this.optionsName, options, this.index, this.parent);
		...
		this.selectedColorSet = typeof (colorSets[this.colorSet]) !== "undefined" ? colorSets[this.colorSet] : colorSets["colorSet1"];
	...
	Chart.prototype._updateOptions = function () {
		...
		if (this.options.zoomEnabled || this.panEnabled) { //panEnabled will be set to true incase of stockchart
			if (this._zoomButton) {
				setCSSProperties(this._zoomButton, { borderRight: this.toolbar.buttonBorderThickness + "px solid " + this.toolbar.buttonBorderColor, backgroundColor: _this.toolbar.itemBackgroundColor, color: _this.toolbar.fontColor });
				...
				addEvent(this._zoomButton, "mousemove", function () {
					if (!preventMouseEvent) {
						var cssStyle = { backgroundColor: _this.toolbar.itemBackgroundColorOnHover, color: _this.toolbar.fontColorOnHover, transition: "0.4s", WebkitTransition: "0.4s" };
						setCSSProperties(_this._zoomButton, cssStyle);
						if (navigator.userAgent.search("MSIE") <= 0)
							setCSSProperties(_this._zoomButton.childNodes[0], { WebkitFilter: "invert(100%)", filter: "invert(100%)" });
					}
					else {
				...
				addEvent(this._zoomButton, "mouseout", function () {
					if (!preventMouseEvent) {
						var cssStyle = { backgroundColor: _this.toolbar.itemBackgroundColor, color: _this.toolbar.fontColor, transition: "0.4s", WebkitTransition: "0.4s" };
						setCSSProperties(_this._zoomButton, cssStyle);
						if (navigator.userAgent.search("MSIE") <= 0)
							setCSSProperties(_this._zoomButton.childNodes[0], { WebkitFilter: "invert(0%)", filter: "invert(0%)" });
					}
				}, this.allDOMEventHandlers);
			}

			if (this._resetButton) {
				setCSSProperties(this._resetButton, { borderRight: this.toolbar.buttonBorderThickness + "px solid " + this.toolbar.buttonBorderColor, backgroundColor: _this.toolbar.itemBackgroundColor, color: _this.toolbar.fontColor });
				...
				addEvent(this._resetButton, "click", function () {
					...
					_this.toolTip.hide();
					if (_this.toolTip && _this.toolTip.enabled)
						_this.toolTip.dispatchEvent("hidden", { chart: _this, toolTip: _this.toolTip }, _this.toolTip);
					...
					if (navigator.userAgent.search("MSIE") <= 0)
						setCSSProperties(_this._resetButton.childNodes[0], { WebkitFilter: "invert(0%)", filter: "invert(0%)" });

					hide(_this._zoomButton, _this._resetButton);

					if (_this.stockChart) {
						_this.stockChart._rangeEventParameter = {
							stockChart: _this.stockChart,
							source: "chart",
							index: _this.stockChart.charts.indexOf(_this),
							minimum: null,
							maximum: null
						};
					}
					...
					if (_this.stockChart) {
						_this.stockChart._rangeEventParameter.type = "rangeChanging";
						_this.stockChart.dispatchEvent("rangeChanging", _this.stockChart._rangeEventParameter, _this.stockChart);
					}
					...
					if (_this.stockChart) {
						_this.stockChart._rangeEventParameter.type = "rangeChanged";
						_this.stockChart.dispatchEvent("rangeChanged", _this.stockChart._rangeEventParameter, _this.stockChart);
					}

				}, this.allDOMEventHandlers);

				addEvent(this._resetButton, "mousemove", function () {
					if (!preventMouseEvent) {
						var cssStyle = { backgroundColor: _this.toolbar.itemBackgroundColorOnHover, color: _this.toolbar.fontColorOnHover, transition: "0.4s", WebkitTransition: "0.4s" };
						setCSSProperties(_this._resetButton, cssStyle);
						if (navigator.userAgent.search("MSIE") <= 0)
							setCSSProperties(_this._resetButton.childNodes[0], { WebkitFilter: "invert(100%)", filter: "invert(100%)" });
					}
				}, this.allDOMEventHandlers);
	...
	Chart.prototype._initialize = function () {
		...
		//this.selectedColorSet = colorSets["colorSet2"];
			...
			for (var series = 0; series < this.options.data.length; series++) {
				...
				if (dataSeries.color === null) {
					if (this.options.data.length > 1) {
						dataSeries._colorSet = [this.selectedColorSet[dataSeries.index % this.selectedColorSet.length]];
						dataSeries.color = this.selectedColorSet[dataSeries.index % this.selectedColorSet.length];
					} else {
						if (dataSeries.type === "line" || dataSeries.type === "stepLine" || dataSeries.type === "spline" || dataSeries.type === "area"
							...
							dataSeries._colorSet = [this.selectedColorSet[0]];
						}
						else
							dataSeries._colorSet = this.selectedColorSet;
					}
				} else {
	...
	Chart.prototype.setLayout = function () {
		...
		if (this._axes.length > 0 && this.options.zoomEnabled && (this.zoomEnabled || this.panEnabled)) {
			for (var i = 0; i < this._axes.length; i++) {
				if (!isNullOrUndefined(this._axes[i].viewportMinimum) || !isNullOrUndefined(this._axes[i].viewportMaximum)) {
					showToolBar = true;
					break;
				}
			}
	...
	Chart.prototype.renderElements = function () {
		...
		if (animations.length > 0) {
			...
			_this._animator.animate(200, _this.animationDuration, function (fractionComplete) {
				...
				_this.dispatchEvent("dataAnimationEnd", {
					chart: _this
				});
			});
		} else {
	...
	Chart.prototype._processStackedPlotUnit = function (plotUnit) {
		...
		for (var j = 0; j < plotUnit.dataSeriesIndexes.length; j++) {
			...
			for (i = 0; i < dataSeries.dataPoints.length; i++) {
				...
				plotUnit.yAbsTotals[dataPointX] = (!plotUnit.yAbsTotals[dataPointX] ? 0 : plotUnit.yAbsTotals[dataPointX]) + Math.abs(dataPointY);
	...
	Chart.prototype.renderCrosshairs = function (axis) {
		for (var i = 0; i < this.axisX.length; i++) {
			if (this.axisX[i] != axis && this.axisX[i].crosshair && this.axisX[i].crosshair.enabled && !this.axisX[i].crosshair._hidden) {
				this.axisX[i].showCrosshair(this.axisX[i].crosshair._updatedValue);
			}
		}

		for (var i = 0; i < this.axisX2.length; i++) {
			if (this.axisX2[i] != axis && this.axisX2[i].crosshair && this.axisX2[i].crosshair.enabled && !this.axisX2[i].crosshair._hidden) {
				this.axisX2[i].showCrosshair(this.axisX2[i].crosshair._updatedValue);
			}
		}

		for (var i = 0; i < this.axisY.length; i++) {
			if (this.axisY[i] != axis && this.axisY[i].crosshair && this.axisY[i].crosshair.enabled && !this.axisY[i].crosshair._hidden) {
				this.axisY[i].showCrosshair(this.axisY[i].crosshair._updatedValue);
			}
		}

		for (var i = 0; i < this.axisY2.length; i++) {
			if (this.axisY2[i] != axis && this.axisY2[i].crosshair && this.axisY2[i].crosshair.enabled && !this.axisY2[i].crosshair._hidden) {
				this.axisY2[i].showCrosshair(this.axisY2[i].crosshair._updatedValue);
			}
		}
	}
	...
	Chart.prototype._mouseEventHandler = function (ev) {
		...
		if (xy.x < plotArea.x1 || xy.x > plotArea.x2 || xy.y < plotArea.y1 || xy.y > plotArea.y2) {
			if (this.toolTip && this.toolTip.enabled) {
				...
				for (var i = 0; i < this.axisX.length; i++) {
					if (this.axisX[i].crosshair && this.axisX[i].crosshair.enabled) {
						this.axisX[i].crosshair.dispatchEvent("hidden", { chart: this, axis: this.axisX[i].options }, this.axisX[i].crosshair);
					}
				}
				for (var i = 0; i < this.axisX2.length; i++) {
					if (this.axisX2[i].crosshair && this.axisX2[i].crosshair.enabled) {
						this.axisX2[i].crosshair.dispatchEvent("hidden", { chart: this, axis: this.axisX2[i].options }, this.axisX2[i].crosshair);
					}
				}
				for (var i = 0; i < this.axisY.length; i++) {
					if (this.axisY[i].crosshair && this.axisY[i].crosshair.enabled) {
						this.axisY[i].crosshair.dispatchEvent("hidden", { chart: this, axis: this.axisY[i].options }, this.axisY[i].crosshair);
					}
				}
				for (var i = 0; i < this.axisY2.length; i++) {
					if (this.axisY2[i].crosshair && this.axisY2[i].crosshair.enabled) {
						this.axisY2[i].crosshair.dispatchEvent("hidden", { chart: this, axis: this.axisY2[i].options }, this.axisY2[i].crosshair);
					}
				}
			} else {
				this.resetOverlayedCanvas();
			...
			if (this.isDrag) {
				...
				if (this.panEnabled || this.zoomEnabled) {
					...
					if (reRender) {
						...
						if (this.stockChart && this.stockChart.navigator && this.stockChart.navigator.enabled) {
							if (!this.stockChart._rangeEventParameter)
	...
	Chart.prototype._plotAreaMouseMove = function (x, y) {
		if (this.isDrag && this.plotInfo.axisPlacement !== "none") {
			...
			if (Math.abs(dragDeltaPX) > 2 && Math.abs(dragDeltaPX) < 8 && (this.panEnabled || this.zoomEnabled)) {
				...
				for (var i = 0; i < this.axisX.length; i++) {
					if (this.axisX[i].crosshair && this.axisX[i].crosshair.enabled) {
						this.axisX[i].crosshair.hide();
						this.axisX[i].crosshair.dispatchEvent("hidden", { chart: this, axis: this.axisX[i].options }, this.axisX[i].crosshair);
					}
				}
				for (var i = 0; i < this.axisX2.length; i++) {
					if (this.axisX2[i].crosshair && this.axisX2[i].crosshair.enabled) {
						this.axisX2[i].crosshair.hide();
						this.axisX2[i].crosshair.dispatchEvent("hidden", { chart: this, axis: this.axisX2[i].options }, this.axisX2[i].crosshair);
					}
				}
				for (var i = 0; i < this.axisY.length; i++) {
					if (this.axisY[i].crosshair && this.axisY[i].crosshair.enabled) {
						this.axisY[i].crosshair.hide();
						this.axisY[i].crosshair.dispatchEvent("hidden", { chart: this, axis: this.axisY[i].options }, this.axisY[i].crosshair);
					}
				}
				for (var i = 0; i < this.axisY2.length; i++) {
					if (this.axisY2[i].crosshair && this.axisY2[i].crosshair.enabled) {
						this.axisY2[i].crosshair.hide();
						this.axisY2[i].crosshair.dispatchEvent("hidden", { chart: this, axis: this.axisY2[i].options }, this.axisY2[i].crosshair);
					}
				}
					...
					_this._panTimerId = setTimeout((function (x1, y1, x2, y2) {
						return function () {
							if (_this._zoomPanToSelectedRegion(x1, y1, x2, y2, true)) {
								...
								if (_this.stockChart && _this.stockChart.navigator && _this.stockChart.navigator.enabled) {
									_this.stockChart._rangeEventParameter.type = "rangeChanging";
									_this.stockChart.dispatchEvent("rangeChanging", _this.stockChart._rangeEventParameter, _this.stockChart);
								}
	...
	Chart.prototype._zoomPanToSelectedRegion = function (px1, py1, px2, py2, keepAxisIndependent) {
		...
		if (validateRegion.isValid) {
			for (var i = 0; i < axesWithValidRange.length; i++) {
				...
				if (this.syncCharts && this.zoomType != "y") {
	...
	Chart.prototype.renderIndexLabels = function (targetCtx) {
		...
		for (var i = 0; i < this._indexLabels.length; i++) {
			...
			var textAlign = getProperty("indexLabelTextAlign", indexLabel.dataPoint, indexLabel.dataSeries);
			var angle = 0;
			...
			if (this.plotInfo.axisPlacement === "normal") {
				...
				x = indexLabel.point.x - (orientation === "horizontal" ? visibleWidth / 2 : visibleWidth / 2 - fontSize / 2);
				...
				if (placement !== "inside") {	//outside or auto
					...
					if (direction > 0) {
						y = indexLabel.point.y + (orientation === "horizontal" ? fontSize / 2 : 0) - visibleHeight - marginY - offset;

						if (y < yMinLimit) {
							if (placement === "auto") {
								y = Math.max(indexLabel.point.y, yMinLimit) + fontSize / 2 + marginY;
							}
							else {
								y = yMinLimit + fontSize / 2 + marginY;
							}
							isInside = y + visibleHeight > indexLabel.point.y;
						}
					}
					else {
						y = indexLabel.point.y + fontSize / 2 + marginY + offset;

						if (y > yMaxLimit - visibleHeight) {
							if (placement === "auto") {
								y = Math.min(indexLabel.point.y, yMaxLimit) + fontSize / 2 - visibleHeight - marginY;
							}
							else {
								y = yMaxLimit + fontSize / 2 - visibleHeight;
							}
					...
					yMaxLimit = Math.min(indexLabel.bounds.y2, plotArea.y2 - visibleHeight + fontSize / 2);


					if (chartTypeLower.indexOf("range") >= 0 || chartTypeLower === "error") {
						if (direction > 0)
							mid = Math.max(indexLabel.bounds.y1, plotArea.y1) + fontSize / 2 + marginY;
						else
							mid = Math.min(indexLabel.bounds.y2, plotArea.y2) + fontSize / 2 - visibleHeight + marginY;
					}
					else
						mid = (Math.max(indexLabel.bounds.y1, plotArea.y1) + Math.min(indexLabel.bounds.y2, plotArea.y2)) / 2 - visibleHeight / 2 + fontSize / 2 + (orientation === "horizontal" ? marginY : 0);

					if (direction > 0) {
						y = Math.max(indexLabel.point.y, mid);
						...
						y = Math.min(indexLabel.point.y, mid);
				...
				y = indexLabel.point.y + fontSize / 2 - visibleHeight / 2 + marginY;
				...
				if (placement !== "inside") {	//outside or auto
					...
					if (direction < 0) {
						x = indexLabel.point.x - (orientation === "horizontal" ? visibleWidth : visibleWidth - fontSize / 2) - marginX - offset;
						...
						x = indexLabel.point.x + (orientation === "horizontal" ? 0 : fontSize / 2) + marginX + offset;

						if (x > xMaxLimit - visibleWidth - marginX - offset) {
							if (placement === "auto") {
								x = Math.min(indexLabel.point.x, xMaxLimit) - (orientation === "horizontal" ? visibleWidth : visibleWidth / 2) - marginX;
							}
							else {
					...
					if (chartTypeLower.indexOf("range") >= 0 || chartTypeLower === "error") {
						if (direction < 0)
							mid = Math.max(indexLabel.bounds.x1, plotArea.x1) + fontSize / 2 + marginX;
						else
							mid = Math.min(indexLabel.bounds.x2, plotArea.x2) - visibleWidth / 2 - marginX + (orientation === "horizontal" ? 0 : fontSize / 2);
					}
					else
						var mid = (Math.max(indexLabel.bounds.x1, plotArea.x1) + Math.min(indexLabel.bounds.x2, plotArea.x2)) / 2 + (orientation === "horizontal" ? 0 : fontSize / 2);

					if (direction < 0) {
						x = Math.max(indexLabel.point.x, mid) - (orientation === "horizontal" ? visibleWidth / 2 : 0);
			...
			if (indexLabelLineThickness && placement !== "inside" &&
				...
				if (chartTypeLower.indexOf("bar") >= 0 || chartTypeLower === "error" && indexLabel.axisSwapped)
					ctx.lineTo(x + (indexLabel.direction > 0 ? -marginX : visibleWidth + marginX) + (orientation === "vertical" ? -fontSize / 2 : 0), y + (orientation === "vertical" ? -visibleHeight / 2 : (visibleHeight / 2 - fontSize / 2)) - marginY);
				else if (chartTypeLower.indexOf("column") >= 0 || chartTypeLower === "error" && !indexLabel.axisSwapped)
					ctx.lineTo(x + visibleWidth / 2 - (orientation === "horizontal" ? 0 : fontSize / 2), y + (orientation === "vertical" ? (y - visibleHeight < indexLabel.point.y ? 0 : -visibleHeight) + marginY : (y - fontSize / 2 < indexLabel.point.y ? visibleHeight : 0) - fontSize / 2));
				else
					ctx.lineTo(x + visibleWidth / 2 - (orientation === "horizontal" ? 0 : fontSize / 2), y + (orientation === "vertical" ? (y - visibleHeight < indexLabel.point.y ? 0 : -visibleHeight) : (y - fontSize / 2 < indexLabel.point.y ? visibleHeight : 0) - fontSize / 2));
				ctx.stroke();
			}
		}
	...
	Chart.prototype.renderRangeArea = function (plotUnit) {
				...
				for (; i < dataPoints.length; i++) {
					...
					if (isFirstDataPointInPlotArea || prevDataNull) {
						...
						if (dataSeries.connectNullData && !isFirstDataPointInPlotArea) {
							...
							closingPath.push({ x: x, y1: y1, y2: y2 });
		...
		function closeArea() {
			...
			for (var i = closingPath.length - 1; i >= 0; i--) {
				...
				ghostCtx.lineTo(point.x, point.y2);
			}
			...
			if (dataSeries.lineThickness > 0) {
				...
				ctx.moveTo(closingPath[0].x, closingPath[0].y1);
				for (var i = 0; i < closingPath.length; i++) {
					point = closingPath[i];
	...
	Chart.prototype.renderPie = function (plotUnit) {
		...
		function initLabels() {
			...
			for (var j = 0; j < dataPoints.length; j++) {
				...
				dataPointEO.indexLabelTextAlign = dataPoint.indexLabelTextAlign ? dataPoint.indexLabelTextAlign : dataSeries.indexLabelTextAlign ? dataSeries.indexLabelTextAlign : "left";
				...
				dataPointEO.indexLabelTextBlock = new TextBlock(_this.plotArea.ctx, {
					...
					textAlign: dataPointEO.indexLabelTextAlign,
	...
	Chart.prototype.renderFunnel = function (plotUnit) {
		...
		function initLabels() {
			...
			for (var i = 0, k = dataPoints.length - 1; i < dataPoints.length || k >= 0; i++, k--) {
				...
				indexLabelTextAlign = dataPoint.indexLabelTextAlign || dataSeries.indexLabelTextAlign || "left";
	...
	Chart.prototype.getPercentAndTotal = function (ds, dp) {
		...
		if (ds.type.indexOf("stacked") >= 0) {
			...
			if (dpX in ds.plotUnit.yTotals) {
				...
				absTotal = ds.plotUnit.yAbsTotals[dpX];

				if (!isNaN(dp.y)) {
					if (absTotal === 0)
						percent = 0;
					else
						percent = (dp.y / absTotal) * 100;
				}
				else
					percent = 0;
			}
	...
	Chart.prototype.replaceKeywordsWithValue = function (str, dp, ds, dpIndex, indexKeywordValue) {
		...
		var fcn = function ($0) {
			...
			if (key === "x") {
				if (ds.axisX && ds.axisX.valueType === "dateTime" || ds.xValueType === "dateTime" || (dp.x && dp.x.getTime)) {
					if (!ds.axisX.logarithmic)
	...
	Legend.prototype.setLayout = function () {
		...
		if (this.verticalAlign === "top" || this.verticalAlign === "bottom") {
			...
			if (this.horizontalAlign === "left" || this.horizontalAlign === "center" || this.horizontalAlign === "right")
	...
	DataSeries.prototype.getDataPointAtX = function (x, findClosest) {
		...
		if (!findClosest && (searchResult.dataPoint.x.getTime ? searchResult.dataPoint.x.getTime() : searchResult.dataPoint.x) === (x.getTime ? x.getTime() : x))
			return searchResult;
	...
	Axis.prototype.createLabels = function () {
			...
			for (var j = firstBreakIndex, i = this.intervalStartPosition; i < endPoint; addToDateTime(i, labelInterval, this.intervalType)) {
				...
				if (!isInBreak) {
					...
					textBlock = new TextBlock(this.ctx, {
						...
						textAlign: this.labelTextAlign,//left, center, right
		...
		if (this.labelAutoFit || this.options.labelAutoFit) {
			...
			if (this._position === "bottom" || this._position === "top") {
				...
				if (!this.chart.panEnabled && this._labels.length >= 1) {
					...
					for (i = autoLabelStartValue; i < this._labels.length; i++) {
						...
						if (!(isNullOrUndefined(this.options.labelAngle) && isNaN(this.options.labelAngle)) || this.options.labelAngle === 0) {//User has set angle -->Rotate
							...
							this.sessionVariables.labelMaxHeight = this.labelWrap ? (labelMaxHeightLimit - (labelEffectiveMaxWidthLimit) * Math.sin(Math.PI / 180 * Math.abs(this.labelAngle))) / Math.cos(Math.PI / 180 * Math.abs(this.labelAngle)) : this.labelFontSize * 1.5;
							...
							if (!isNullOrUndefined(this.options.labelWrap)) {//User has set Wrap (true/false)
								if (this.labelWrap) {//wrap is true -->Wrap
									if (!isNullOrUndefined(this.options.labelMaxWidth)) {//User has set labelMaxWidth -->Wrap after user set labelMaxWidth
										...
										this.sessionVariables.labelWrap = this.labelWrap;
										if (sizeNext && ((size.width + sizeNext.width) >> 0) > (2 * labelMaxWidth)) {
				...
				//Panning Mode
					...
					for (i = 0; i < this._labels.length; i++) {
						textBlock = this._labels[i].textBlock;
						textBlock.maxWidth = this.labelMaxWidth = isNullOrUndefined(this.options.labelMaxWidth) ? (!isNullOrUndefined(this.sessionVariables.labelMaxWidth) ? this.sessionVariables.labelMaxWidth : this.sessionVariables.labelMaxWidth = labelMaxWidth) : this.options.labelMaxWidth;
						textBlock.fontSize = this.labelFontSize = isNullOrUndefined(this.options.labelFontSize) ? (!isNullOrUndefined(this.sessionVariables.labelFontSize) ? this.sessionVariables.labelFontSize : this.sessionVariables.labelFontSize = this.labelFontSize) : this.options.labelFontSize;
						textBlock.angle = this.labelAngle = isNullOrUndefined(this.options.labelAngle) ? (!isNullOrUndefined(this.sessionVariables.labelAngle) ? this.sessionVariables.labelAngle : this.sessionVariables.labelAngle = this.labelAngle) : this.labelAngle;
						textBlock.wrap = this.labelWrap = isNullOrUndefined(this.options.labelWrap) ? (!isNullOrUndefined(this.sessionVariables.labelWrap) ? this.sessionVariables.labelWrap : this.sessionVariables.labelWrap = this.labelWrap) : this.options.labelWrap;
						textBlock.maxHeight = (!isNullOrUndefined(this.sessionVariables.labelMaxHeight) ? this.sessionVariables.labelMaxHeight : (this.sessionVariables.labelMaxHeight = labelMaxHeight));
						textBlock.measureText();
					}
				}
			}
		...
		for (var i = 0; i < this.stripLines.length; i++) {
			...
			if (stripLine.labelPlacement === "outside") {
				...
				if (this._position === "bottom" || this._position === "top") {
					if (isNullOrUndefined(stripLine.options.labelWrap) && !isNullOrUndefined(this.sessionVariables.stripLineLabelMaxHeight)) {
						stripLineLabelMaxHeight = this.sessionVariables.stripLineLabelMaxHeight;
					}
					else {
						this.sessionVariables.stripLineLabelMaxHeight = stripLineLabelMaxHeight = stripLine.labelWrap ? this.chart.height * .8 >> 0 : this.labelFontSize * 1.5;
					}
				}
				if (this._position === "left" || this._position === "right") {
					if (isNullOrUndefined(stripLine.options.labelWrap) && !isNullOrUndefined(this.sessionVariables.stripLineLabelMaxHeight)) {
						...
						this.sessionVariables.stripLineLabelMaxHeight = stripLineLabelMaxHeight = stripLine.labelWrap ? this.chart.width * .8 >> 0 : this.labelFontSize * 1.5;
					}
				}
	...
	Axis.prototype.createLabelsAndCalculateWidth = function () {
		...
		if (this._position === "left" || this._position === "right") {
			...
			this.createLabels();
			if (this.labelPlacement != "inside" || (this.labelPlacement === "inside" && this._index > 0)) {
				for (i = 0; i < this._labels.length; i++) {
		...
		var axisWidth = titleHeight + maxLabelEffectiveWidth + (this.tickPlacement === "inside" ? 0 : this.tickLength) + padding;
	...
	Axis.prototype.createLabelsAndCalculateHeight = function () {
		...
		var axisHeight = titleHeight + maxLabelEffectiveHeight + (this.tickPlacement === "inside" ? 0 : this.tickLength) + padding;
		...
		if (axisPlacement === "normal") {
				...
				if (axisX && axisX.length > 0) {
					for (var k = 0; k < axisX.length; k++) {
						if (axisX[k]) {
							...
							if (axisX[k] && axisX[k].labelAutoFit && !isNullOrUndefined(firstLabelPosition) && !isNullOrUndefined(lastLabelPosition) && !chart.isNavigator && !chart.stockChart) {
							...
							if (!chart.panEnabled)
								...
								axisXTotalHeight = !isNullOrUndefined(chart.sessionVariables.axisX.height) ? chart.sessionVariables.axisX.height : (chart.sessionVariables.axisX.height = axisXTotalHeight);
							...
							axisX[k].lineCoordinates.y2 = y1 + axisX[k].lineThickness / 2;

							if (axisX[k].labelPlacement === "inside" && k > 0) {
								axisX[k].lineCoordinates.y1 = axisX[k].lineCoordinates.y1 + axisXHeight[k] - (axisX[k]._titleTextBlock ? axisX[k]._titleTextBlock.height : 0) - (axisX[k].tickPlacement === "inside" ? axisX[k].tickLength : 0);
								axisX[k].lineCoordinates.y2 = axisX[k].lineCoordinates.y1 + axisX[k].lineThickness / 2;
							}
				...
				if (axisX2 && axisX2.length > 0) {
					for (var k = 0; k < axisX2.length; k++) {
						...
						if (!chart.panEnabled)
							...
							axisX2TotalHeight = !isNullOrUndefined(chart.sessionVariables.axisX2.height) ? chart.sessionVariables.axisX2.height : (chart.sessionVariables.axisX2.height = axisX2TotalHeight);
						...
						if (axisX2[k].labelPlacement === "inside" && k > 0) {
							axisX2[k].lineCoordinates.y1 = axisX2[k - 1].bounds.y1 - axisX2Height[k] + (axisX2[k]._titleTextBlock ? axisX2[k]._titleTextBlock.height : 0);
						}
					...
					for (var k = 0; k < axisY.length; k++) {
						...
						if (axisY[k]) {
							...
							axisY[k].lineCoordinates = {
								x1: x1 - prevAxisYWidth, y1: y1, x2: x2 - prevAxisYWidth, y2: y2, height: Math.abs(y2 - y1)
							}

							if (axisY[k].labelPlacement === "inside" && k > 0) {
								axisY[k].lineCoordinates.x1 = axisY[k].lineCoordinates.x1 - (axisYWidth[k] - (axisY[k]._titleTextBlock ? axisY[k]._titleTextBlock.height : 0)) + (axisY[k].tickPlacement === "outside" ? axisY[k].tickLength : 0);
								axisY[k].lineCoordinates.x2 = axisY[k].lineCoordinates.x1 + axisY[k].lineThickness / 2;
							}
				...
				if (axisY2 && axisY2.length > 0) {
					for (var k = 0; k < axisY2.length; k++) {
						...
						if (axisY2[k]) {
							...
							if (axisY2[k].labelPlacement === "inside" && k > 0) {
								axisY2[k].lineCoordinates.x1 = axisY2[k].lineCoordinates.x1 + (axisY2Width[k] - (axisY2[k]._titleTextBlock ? axisY2[k]._titleTextBlock.height : 0)) - (axisY2[k].tickPlacement === "outside" ? axisY2[k].tickLength : 0) - 2;
								axisY2[k].lineCoordinates.x2 = axisY2[k].lineCoordinates.x1 + axisY2[k].lineThickness / 2;
							}
			...
			while (i < 4) {
				...
				if (!chart.panEnabled)
					...
					for (var k = 0; k < axisY.length; k++)
						axisYHeight[k] = !isNullOrUndefined(chart.sessionVariables.axisY.height) ? chart.sessionVariables.axisY.height : (chart.sessionVariables.axisY.height = axisYHeight[k]);
				...
				if (axisY && axisY.length > 0) {
					for (var k = axisY.length - 1; k >= 0; k--) {
						...
						y2 = Math.round(freeSpace.y2 > axisY[k].chart.height ? axisY[k].chart.height : freeSpace.y2);
						...
						if (axisY[k].labelPlacement === "inside" && k > 0) {
							axisY[k].lineCoordinates.y1 = axisY[k].lineCoordinates.y1 + axisYHeight[k] - (axisY[k]._titleTextBlock ? axisY[k]._titleTextBlock.height : 0) - (axisY[k].tickPlacement === "inside" ? axisY[k].tickLength : 0);
							axisY[k].lineCoordinates.y2 = axisY[k].lineCoordinates.y1 + axisY[k].lineThickness / 2;
						}
				...
				if (axisY2 && axisY2.length > 0) {
					for (var k = axisY2.length - 1; k >= 0; k--) {
						if (axisY2[k]) {
							...
							if (axisY2[k].labelPlacement === "inside" && k > 0) {
								axisY2[k].lineCoordinates.y1 = y2 - axisY2Height[k] + (axisY2[k]._titleTextBlock ? axisY2[k]._titleTextBlock.height : 0);
							}
				...
				if (axisX && axisX.length > 0) {
					for (var k = 0; k < axisX.length; k++) {
						...
						if (axisX[k].labelPlacement === "inside" && k > 0) {
							axisX[k].lineCoordinates.x1 = axisX[k].lineCoordinates.x1 - (axisXWidth[k] - (axisX[k]._titleTextBlock ? axisX[k]._titleTextBlock.height : 0)) + (axisX[k].tickPlacement === "outside" ? axisX[k].tickLength : 0);
							axisX[k].lineCoordinates.x2 = axisX[k].lineCoordinates.x1 + axisX[k].lineThickness / 2;
						}
				...
				if (axisX2 && axisX2.length > 0) {
					for (var k = 0; k < axisX2.length; k++) {
						...
						if (axisX2[k].labelPlacement === "inside" && k > 0) {
							axisX2[k].lineCoordinates.x1 = axisX2[k].lineCoordinates.x1 + (axisX2Width[k] - (axisX2[k]._titleTextBlock ? axisX2[k]._titleTextBlock.height : 0) - 2) - (axisX2[k].tickPlacement === "outside" ? axisX2[k].tickLength : 0);
							axisX2[k].lineCoordinates.x2 = axisX2[k].lineCoordinates.x1 + axisX2[k].lineThickness / 2;
						}

						axisX2[k].bounds = {
							x1: axisX2[k].lineCoordinates.x1, y1: y1, x2: x2 + axisX2Width[k] + prevAxisX2Width, y2: y2, width: x2 - x1, height: y2 - y1
						};
	...
	Axis.prototype.calculateStripLinesThicknessInValues = function () {
		for (var i = 0; i < this.stripLines.length; i++)
			if (this.stripLines[i].startValue !== null && this.stripLines[i].endValue !== null) {
				...
				if (this.logarithmic)
					this.stripLines[i].value *= Math.sqrt(Math.log(Math.max(this.stripLines[i].startValue, this.stripLines[i].endValue) / Math.min(this.stripLines[i].startValue, this.stripLines[i].endValue)) / Math.log(valueDiff));
				else
	...
	Axis.prototype.renderLabelsTicksAndTitle = function () {
		...
		if (this._position === "bottom") {
			for (i = 0; i < this._labels.length; i++) {
				...
				if (this.tickThickness && this.tickPlacement != "inside") {
				...
				if (label.textBlock.angle === 0) {
					...
					xy.y = this.labelPlacement === "inside" ? xy.y - ((this.tickPlacement === "inside" ? this.tickLength : 0) + label.textBlock.height - label.textBlock.fontSize / 2) : xy.y + (this.tickPlacement === "inside" ? 0 : this.tickLength) + label.textBlock.fontSize / 2 + 5;
					...
					xy.y = this.labelPlacement === "inside" ? (this.labelAngle < 0 ? xy.y - (this.tickPlacement === "inside" ? this.tickLength : 0) - 5 : xy.y - (this.tickPlacement === "inside" ? this.tickLength : 0) - Math.abs((this.labelAngle < 0 ? label.textBlock.width * Math.sin(Math.PI / 180 * this.labelAngle) + 5 : label.textBlock.width * Math.sin(Math.PI / 180 * this.labelAngle) + 5))) : (xy.y + (this.tickPlacement === "inside" ? 0 : this.tickLength) + Math.abs((this.labelAngle < 0 ? label.textBlock.width * Math.sin(Math.PI / 180 * this.labelAngle) - 5 : 5)));
				}
			...
			for (i = 0; i < this._labels.length; i++) {
				...
				if (label.textBlock.angle === 0) {
					xy.x -= label.textBlock.width / 2;
					xy.y = this.labelPlacement === "inside" ? xy.y + this.labelFontSize / 2 + (this.tickPlacement === "inside" ? this.tickLength : 0) + 5 : xy.y - ((this.tickPlacement === "inside" ? 0 : this.tickLength) + label.textBlock.height - label.textBlock.fontSize / 2);
				} else {
					...
					xy.x = this.labelPlacement === "inside" ? (this.labelAngle > 0 ? xy.x : xy.x - (label.textBlock.width * Math.cos(Math.PI / 180 * this.labelAngle))) : xy.x + ((label.textBlock.height - this.labelFontSize) * Math.sin(Math.PI / 180 * this.labelAngle)) - (this.labelAngle > 0 ? label.textBlock.width * Math.cos(Math.PI / 180 * this.labelAngle) : 0);
					xy.y = this.labelPlacement === "inside" ? (this.labelAngle > 0 ? xy.y + (this.tickPlacement === "inside" ? this.tickLength : 0) + 5 : xy.y - label.textBlock.width * Math.sin(Math.PI / 180 * this.labelAngle) + (this.tickPlacement === "inside" ? this.tickLength : 0) + 5) : xy.y - ((this.tickPlacement === "inside" ? 0 : this.tickLength) + ((label.textBlock.height - label.textBlock.fontSize / 2) * Math.cos(Math.PI / 180 * this.labelAngle) + (this.labelAngle > 0 ? label.textBlock.width * Math.sin(Math.PI / 180 * this.labelAngle) : 0)));
				}
				label.textBlock.x = xy.x;
				label.textBlock.y = xy.y;
			...
			for (i = 0; i < this._labels.length; i++) {
				...
				if (this.labelAngle === 0) {
					label.textBlock.y = xy.y;
					label.textBlock.x = this.labelPlacement === "inside" ? xy.x + (this.tickPlacement === "inside" ? this.tickLength : 0) + 5 : xy.x - (label.textBlock.width * Math.cos(Math.PI / 180 * this.labelAngle)) - (this.tickPlacement === "inside" ? 0 : this.tickLength) - 5;
				} else {
					label.textBlock.y = this.labelPlacement === "inside" ? xy.y : (xy.y - (label.textBlock.width * Math.sin(Math.PI / 180 * this.labelAngle)));
					label.textBlock.x = this.labelPlacement === "inside" ? (xy.x + (this.tickPlacement === "inside" ? this.tickLength : 0) + 5) : (this.labelAngle > 0 ? (xy.x - (label.textBlock.width * Math.cos(Math.PI / 180 * this.labelAngle)) - (this.tickPlacement === "inside" ? 0 : this.tickLength) - 5) : (xy.x - (label.textBlock.width * Math.cos(Math.PI / 180 * this.labelAngle)) + (label.textBlock.height - label.textBlock.fontSize / 2 - 5) * Math.sin(Math.PI / 180 * this.labelAngle) - (this.tickPlacement === "inside" ? 0 : this.tickLength)));
				}
			}
			...
			for (i = 0; i < this._labels.length; i++) {
				...
				if (this.labelAngle === 0) {
					label.textBlock.y = xy.y;
					label.textBlock.x = this.labelPlacement === "inside" ? xy.x - label.textBlock.width - (this.tickPlacement === "inside" ? this.tickLength : 0) - 5 : xy.x + (this.tickPlacement === "inside" ? 0 : this.tickLength) + 5;
				} else {
					...
					label.textBlock.x = this.labelPlacement === "inside" ? (xy.x - (label.textBlock.width * Math.cos(Math.PI / 180 * this.labelAngle)) - (this.tickPlacement === "inside" ? this.tickLength : 0) - 5) : (this.labelAngle > 0 ? (xy.x + ((label.textBlock.height - label.textBlock.fontSize / 2 - 5) * Math.sin(Math.PI / 180 * this.labelAngle)) + (this.tickPlacement === "inside" ? 0 : this.tickLength)) : xy.x + (this.tickPlacement === "inside" ? 0 : this.tickLength) + 5);
				}
			}
	...
	Axis.prototype.renderStripLinesOfThicknessType = function (thicknessType) {
		...
		for (i = 0; i < this.stripLines.length; i++) {
			...
			if (thicknessType === "value" && ((stripLine.startValue <= this.viewportMinimum && stripLine.endValue <= this.viewportMinimum) || (stripLine.startValue >= this.viewportMaximum && stripLine.endValue >= this.viewportMaximum) || isNullOrUndefined(stripLine.startValue) || isNullOrUndefined(stripLine.endValue) || isNaN(this.range)))
				continue;
	...
	Axis.prototype.renderCrosshair = function (mouseX, mouseY) {
		if (!(isFinite(this.minimum) && isFinite(this.maximum)))
			return;
		...
		this.crosshair.dispatchEvent("updated", parameters, this);
	}

	Axis.prototype.showCrosshair = function (value) {
		if (isNullOrUndefined(value) || (value < this.viewportMinimum || value > this.viewportMaximum))
			return;

		if (this._position === "top" || this._position === "bottom")
			this.crosshair.render(this.convertValueToPixel(value), null, value);
		else
			this.crosshair.render(null, this.convertValueToPixel(value), value);
	}
	...
	Axis.prototype.calculateLogarithmicAxisParameters = function () {
		...
		if (this.minimum === null || this.maximum === null) {
			...
			if (this.minimum === null) {
				...
				if (!isNullOrUndefined(this.sessionVariables.viewportMinimum === null || isNaN(this.sessionVariables.viewportMinimum) ? typeof this.sessionVariables.newViewportMinimum === "undefined" ? Infinity : this.sessionVariables.newViewportMinimum : this.sessionVariables.viewportMinimum))
			...
			if (this.maximum === null) {
				...
				if (!isNullOrUndefined(this.sessionVariables.viewportMaximum === null || isNaN(this.sessionVariables.viewportMaximum) ? typeof this.sessionVariables.newViewportMaximum === "undefined" ? 0 : this.sessionVariables.newViewportMaximum : this.sessionVariables.viewportMaximum))
	...
	StripLine.prototype.render = function () {
		...
		var lineWidth = Math.abs(this._thicknessType === "pixel" ? this.thickness : Math.abs(this.parent.convertValueToPixel(this.endValue) - this.parent.convertValueToPixel(this.startValue)));
	...
	Crosshair.prototype.showAt = function (value) {
		if (!this.enabled)
			return false;
		...
		chart.clearedOverlayedCanvas = this.parent.type;

		if (chart.plotInfo.axisPlacement === "xySwapped") {
			if (this.parent._position === "bottom") {
				for (var i = 0; i < chart.axisY.length; i++) {
					if (this.parent === chart.axisY[i]) {
						if (value >= chart.axisY[i].viewportMinimum && value <= chart.axisY[i].viewportMaximum) {
							chart.axisY[i]._crosshairValue = value;
						}
						else {
							chart.axisY[i]._crosshairValue = null;
						}
					}
				}
			}
				...
				for (var i = 0; i < chart.axisY2.length; i++) {
					if (this.parent === chart.axisY2[i]) {
						if (value >= chart.axisY2[i].viewportMinimum && value <= chart.axisY2[i].viewportMaximum) {
							chart.axisY2[i]._crosshairValue = value;
						}
						else {
							chart.axisY2[i]._crosshairValue = null;
						}
					}
				}
			}
			else if (this.parent._position === "left") {
				for (var i = 0; i < chart.axisX.length; i++) {
					if (this.parent === chart.axisX[i]) {
						if (value >= chart.axisX[i].viewportMinimum && value <= chart.axisX[i].viewportMaximum) {
							chart.axisX[i]._crosshairValue = value;
						}
						else {
							chart.axisX[i]._crosshairValue = null;
						}
					}
				}
			}
				...
				for (var i = 0; i < chart.axisX2.length; i++) {
					if (this.parent === chart.axisX2[i]) {
						if (value >= chart.axisX2[i].viewportMinimum && value <= chart.axisX2[i].viewportMaximum) {
							chart.axisX2[i]._crosshairValue = value;
						}
						else {
							chart.axisX2[i]._crosshairValue = null;
						}
					}
				}
			}
		...
		for (var i = 0; i < chart.axisX.length; i++) {
			var crosshairValue = chart.axisX[i]._crosshairValue;
			if (chart.axisX[i].crosshair && chart.axisX[i].crosshair.enabled && !isNullOrUndefined(crosshairValue) && crosshairValue >= chart.axisX[i].viewportMinimum && crosshairValue <= chart.axisX[i].viewportMaximum) {
				chart.axisX[i].showCrosshair(crosshairValue);
				chart.axisX[i].crosshair._updatedValue = crosshairValue;
				if (this === chart.axisX[i].crosshair)
					shown = true;
			}
		}

		for (var i = 0; i < chart.axisX2.length; i++) {
			var crosshairValue = chart.axisX2[i]._crosshairValue;
			if (chart.axisX2[i].crosshair && chart.axisX2[i].crosshair.enabled && !isNullOrUndefined(crosshairValue) && crosshairValue >= chart.axisX2[i].viewportMinimum && crosshairValue <= chart.axisX2[i].viewportMaximum) {
				chart.axisX2[i].showCrosshair(crosshairValue);
				chart.axisX2[i].crosshair._updatedValue = crosshairValue;
				if (this === chart.axisX2[i].crosshair)
					shown = true;
			}
		}

		for (var i = 0; i < chart.axisY.length; i++) {
			var crosshairValue = chart.axisY[i]._crosshairValue;
			if (chart.axisY[i].crosshair && chart.axisY[i].crosshair.enabled && !isNullOrUndefined(crosshairValue) && crosshairValue >= chart.axisY[i].viewportMinimum && crosshairValue <= chart.axisY[i].viewportMaximum) {
				chart.axisY[i].showCrosshair(crosshairValue);
				chart.axisY[i].crosshair._updatedValue = crosshairValue;
				if (this === chart.axisY[i].crosshair)
					shown = true;
			}
		}

		for (var i = 0; i < chart.axisY2.length; i++) {
			var crosshairValue = chart.axisY2[i]._crosshairValue;
			if (chart.axisY2[i].crosshair && chart.axisY2[i].crosshair.enabled && !isNullOrUndefined(crosshairValue) && crosshairValue >= chart.axisY2[i].viewportMinimum && crosshairValue <= chart.axisY2[i].viewportMaximum) {
				chart.axisY2[i].showCrosshair(crosshairValue);
				chart.axisY2[i].crosshair._updatedValue = crosshairValue;
				if (this === chart.axisY2[i].crosshair)
					shown = true;
			}
		}

		if (this.chart.toolTip && this.chart.toolTip._entries)
	...
	Crosshair.prototype.render = function (mouseX, mouseY, label) {
		var x1, x2, y1, y2;
		...
		this.labelFontSize = Math.abs(isNullOrUndefined(this.options.labelFontSize) ? this.parent.labelFontSize : this.labelFontSize);
		...
		if (this.snapToDataPoint) {
			...
			if (this.chart.plotInfo.axisPlacement === "xySwapped") {
				...
				for (var i = 0; i < this.parent.dataSeries.length; i++) {
					...
					if (entry && entry.index >= 0) {
						...
						if (entry.dataPoint.y !== null && entry.dataSeries.visible)
				...
				if (this.parent._position === "bottom" || this.parent._position === "top") {
					...
					if (this.parent.dataSeries[index].type === "rangeBar" || this.parent.dataSeries[index].type === "error") {
						...
						this.value = closest.dataPoint.y[dpsIndex];
						textBlock.text = this.labelFormatter ? this.labelFormatter({ chart: this.chart, axis: this.parent.options, crosshair: this.options, value: closest.dataPoint.y[dpsIndex] }) : !isNullOrUndefined(this.options.label) ? this.label : numberFormat(label ? label : closest.dataPoint.y[dpsIndex], this.valueFormatString, this.chart._cultureInfo);
					}
						...
						textBlock.text = this.labelFormatter ? this.labelFormatter({ chart: this.chart, axis: this.parent.options, crosshair: this.options, value: closest.dataPoint.y }) : !isNullOrUndefined(this.options.label) ? this.label : numberFormat(label ? label : closest.dataPoint.y, this.valueFormatString, this.chart._cultureInfo);
					}
						...
						this.value = yPercent;
						textBlock.text = this.labelFormatter ? this.labelFormatter({ chart: this.chart, axis: this.parent.options, crosshair: this.options, value: yPercent }) : !isNullOrUndefined(this.options.label) ? this.label : numberFormat(label ? label : yPercent, this.valueFormatString, this.chart._cultureInfo);
					}
					else {
						...
						this.value = closest.dataPoint.y;
						textBlock.text = this.labelFormatter ? this.labelFormatter({ chart: this.chart, axis: this.parent.options, crosshair: this.options, value: closest.dataPoint.y }) : !isNullOrUndefined(this.options.label) ? this.label : numberFormat(label ? label : closest.dataPoint.y, this.valueFormatString, this.chart._cultureInfo);
					}
					...
					textBlock.y = this.parent.lineCoordinates.y2 + (this.parent._position === "top" ? -textBlock.height + this.parent.tickLength : textBlock.fontSize / 2) + 2;
					if ((textBlock.y + textBlock.height) > this.chart.bounds.y2) {
						textBlock.y = this.chart.bounds.y2 - textBlock.height;
					}
					else if (textBlock.y < this.chart.bounds.y1) {
						textBlock.y = this.chart.bounds.y1;
					}
				}
				else if (this.parent._position === "left" || this.parent._position === "right") {
					...
					this.value = closest.dataPoint.x;
				...
				if (this.parent._position === "bottom" || this.parent._position === "top") {
					...
					if (this.parent.dataSeries[index].type === "rangeColumn" || this.parent.dataSeries[index].type === "rangeArea" || this.parent.dataSeries[index].type === "error" || this.parent.dataSeries[index].type === "rangeSplineArea"
						...
						textBlock.text = this.labelFormatter ? this.labelFormatter({ chart: this.chart, axis: this.parent.options, crosshair: this.options, value: closest.dataSeries.dataPointEOs[closest.index].cumulativeSum }) : !isNullOrUndefined(this.options.label) ? this.label : numberFormat(label ? label : closest.dataSeries.dataPointEOs[closest.index].cumulativeSum, this.valueFormatString, this.chart._cultureInfo);
						this.value = closest.dataSeries.dataPointEOs[closest.index].cumulativeSum;
					}
					else {
						stripY = (overlaidCanvasCtx.lineWidth % 2 === 1) ? (isNullOrUndefined(mouseX) ? mouseY : this.parent.convertValueToPixel(closest.dataPoint.y) << 0) + .5 : (isNullOrUndefined(mouseX) ? mouseY : this.parent.convertValueToPixel(closest.dataPoint.y) << 0);

						textBlock.text = this.labelFormatter ? this.labelFormatter({ chart: this.chart, axis: this.parent.options, crosshair: this.options, value: label ? label : closest.dataPoint.y }) : !isNullOrUndefined(this.options.label) ? this.label : numberFormat(label ? label : closest.dataPoint.y, this.valueFormatString, this.chart._cultureInfo);
						this.value = closest.dataPoint.y;
					}
			...
			if (this.parent._position === "bottom" || this.parent._position === "top") {
				if (this.parent._position === "top" && ((textBlock.y - textBlock.fontSize / 2) < this.chart.bounds.y1))
					textBlock.y = this.chart.bounds.y1 + textBlock.fontSize / 2;
				if (this.parent._position === "bottom" && (this.parent.lineCoordinates.y2 - textBlock.fontSize / 2 + textBlock.measureText().height) > this.chart.bounds.y2)
					textBlock.y = this.chart.bounds.y2 - textBlock.height + textBlock.fontSize / 2 + 2;

				if (x1 >= this.parent.convertValueToPixel(this.parent.reversed ? this.parent.viewportMaximum : this.parent.viewportMinimum) && x2 <= this.parent.convertValueToPixel(this.parent.reversed ? this.parent.viewportMinimum : this.parent.viewportMaximum)) {
					if (lineWidth > 0) {
					...
					//Hide Crosshair - Trigger "hidden"
				}
			}

			if (this.parent._position === "left" || this.parent._position === "right") {
				if (this.parent._position === "left" && textBlock.x < this.chart.bounds.x1)
					textBlock.x = this.chart.bounds.x1;
				if (this.parent._position === "right" && (textBlock.x + textBlock.measureText().width) > this.chart.bounds.x2)
					textBlock.x = this.chart.bounds.x2 - textBlock.measureText().width;

				if (y2 >= this.parent.convertValueToPixel(this.parent.reversed ? this.parent.viewportMinimum : this.parent.viewportMaximum) && y1 <= this.parent.convertValueToPixel(this.parent.reversed ? this.parent.viewportMaximum : this.parent.viewportMinimum)) {
					if (lineWidth > 0) {
						overlaidCanvasCtx.moveTo(x1, y1);
						overlaidCanvasCtx.lineTo(x2, y2);
						overlaidCanvasCtx.stroke();
			...
			if (this.chart.plotInfo.axisPlacement === "xySwapped") {
				if (this.parent._position === "left" || this.parent._position === "right") {
					...
					if (hasAllLabels) {
						if (this.parent.type === "axisX") {
							...
							for (var i = 0; i < this.parent.dataSeries.length; i++) {
								...
								if (entry && entry.index >= 0) {
									textBlock.text = this.labelFormatter ? this.labelFormatter({ chart: this.chart, axis: this.parent.options, crosshair: this.options, value: label ? label : this.parent.convertPixelToValue(mouseX) }) : !isNullOrUndefined(this.options.label) ? this.label : entry.dataPoint.label;
								}
							}
						}
					}
						...
						textBlock.text = this.labelFormatter ? this.labelFormatter({ chart: this.chart, axis: this.parent.options, crosshair: this.options, value: label ? label : (this.parent.convertPixelToValue(mouseY)) }) : !isNullOrUndefined(this.options.label) ? this.label : dateFormat(label ? label : (this.parent.convertPixelToValue(mouseY)), this.valueFormatString, this.chart._cultureInfo);
					}
					else if (this.parent.valueType === "number") {
						textBlock.text = this.labelFormatter ? this.labelFormatter({ chart: this.chart, axis: this.parent.options, crosshair: this.options, value: label ? label : this.parent.convertPixelToValue(mouseY) }) : !isNullOrUndefined(this.options.label) ? this.label : numberFormat(label ? label : this.parent.convertPixelToValue(mouseY), this.valueFormatString, this.chart._cultureInfo);
					}
					...
					textBlock.text = this.labelFormatter ? this.labelFormatter({ chart: this.chart, axis: this.parent.options, crosshair: this.options, value: label ? label : this.parent.convertPixelToValue(mouseX) }) : !isNullOrUndefined(this.options.label) ? this.label : numberFormat(label ? label : this.parent.convertPixelToValue(mouseX), this.valueFormatString, this.chart._cultureInfo);
					...
					if (hasAllLabels) {
						if (this.parent.type === "axisX") {
							...
							for (var i = 0; i < this.parent.dataSeries.length; i++) {
								...
								if (entry && entry.index >= 0) {
									textBlock.text = this.labelFormatter ? this.labelFormatter({ chart: this.chart, axis: this.parent.options, crosshair: this.options, value: label ? label : this.parent.convertPixelToValue(mouseX) }) : !isNullOrUndefined(this.options.label) ? this.label : (label ? label : entry.dataPoint.label);
								}
							}
						}
					}
						...
						textBlock.text = this.labelFormatter ? this.labelFormatter({ chart: this.chart, axis: this.parent.options, crosshair: this.options, value: label ? label : this.parent.dataSeries.length > 0 ? this.parent.convertPixelToValue(mouseX) : "" }) : !isNullOrUndefined(this.options.label) ? this.label : numberFormat(label ? label : this.parent.convertPixelToValue(mouseX), this.valueFormatString, this.chart._cultureInfo);
					}
			...
			if (this.parent._position === "left" && textBlock.x < this.chart.bounds.x1)
				textBlock.x = this.chart.bounds.x1;
			else if (this.parent._position === "right" && (textBlock.x + textBlock.measureText().width) > this.chart.bounds.x2)
				textBlock.x = this.chart.bounds.x2 - textBlock.measureText().width;
			else if (this.parent._position === "top" && (textBlock.y - textBlock.fontSize / 2) < this.chart.bounds.y1)
				textBlock.y = this.chart.bounds.y1 + textBlock.fontSize / 2;
			else if (this.parent._position === "bottom" && (this.parent.lineCoordinates.y2 - textBlock.fontSize / 2 + textBlock.measureText().height) > this.chart.bounds.y2)
				textBlock.y = this.chart.bounds.y2 - textBlock.height + textBlock.fontSize / 2 + 2;
			...
			if (this.parent._position === "bottom" || this.parent._position === "top")
				this.value = this.parent.convertPixelToValue(mouseX);
			else
				this.value = this.parent.convertPixelToValue(mouseY);
		}

		if (this.parent._position === "bottom" || this.parent._position === "top")
			this._updatedValue = this.parent.convertPixelToValue(stripX);
		if (this.parent._position === "left" || this.parent._position === "right")
			this._updatedValue = this.parent.convertPixelToValue(stripY);
	...
	ToolTip.prototype._updateToolTip = function (mouseX, mouseY, closest) {
		...
		if (entries.length > 0) {
			...
			if (this.enabled) {
				...
				if (toolTipInnerHtml !== null) {
					...
					this.positionLeft = toolTipLeft;
					this.positionBottom = toolTipBottom;
				...
				for (var i = 0; i < entries.length; i++) {
					paramEntries.push({
						xValue: entries[i].dataPoint.x,
						dataPoint: entries[i].dataPoint,
						dataSeries: entries[i].dataSeries,
						dataPointIndex: entries[i].index,
						dataSeriesIndex: entries[i].dataSeries._index
					});
				}
				var parameters = {
					chart: this.chart,
					toolTip: this.options,
					content: toolTipInnerHtml,
					entries: paramEntries
				};

				this._entries = entries;
				this.dispatchEvent("updated", parameters, this);
			}
		}
		else {
			this.hide();
		...
		if (this.chart.axisX) {
			for (var k = 0; k < this.chart.axisX.length; k++) {
				x = this.chart.axisX[k].convertPixelToValue({ x: this.chart.plotInfo.axisPlacement === "xySwapped" ? mouseY : mouseX });
				var entry = null;
				for (var i = 0; i < this.chart.axisX[k].dataSeries.length; i++) {
					entry = this.chart.axisX[k].dataSeries[i].getDataPointAtX(x, closest);
		...
		if (this.chart.axisX2) {
			for (var k = 0; k < this.chart.axisX2.length; k++) {
				x = this.chart.axisX2[k].convertPixelToValue({ x: this.chart.plotInfo.axisPlacement === "xySwapped" ? mouseY : mouseX });
				var entry = null;
				for (var i = 0; i < this.chart.axisX2[k].dataSeries.length; i++) {
					entry = this.chart.axisX2[k].dataSeries[i].getDataPointAtX(x, closest);
		...
		var parameters = {
			...
			toolTip: this,
			content: toolTipInnerHtml,
			dataSeries: updatedEntries[0].dataSeries,
			dataSeriesIndex: updatedEntries[0].dataSeries._index,
			dataPoint: updatedEntries[0].dataPoint,
			entries: updatedEntries,
			x: updatedEntries[0].dataPoint.x
		};
	...
	ToolTip.prototype.highlightObjects = function (entries) {
		...
		var overlaidCanvasCtx = this.chart.overlaidCanvasCtx;
		if (isNullOrUndefined(this.chart.clearedOverlayedCanvas) || this.chart.clearedOverlayedCanvas === "toolTip") {
			this.chart.resetOverlayedCanvas();
	...
	ToolTip.prototype.showAtIndex = function (dataPointIndex, dataSeriesIndex) {
		return;
	}

	ToolTip.prototype.showAtX = function (xValue, dataSeriesIndex) {
		if (!this.enabled)
			return false;
		this.chart.clearedOverlayedCanvas = null;

		var px, py, dataSeries, dataPoint, entry, valueOutsideViewport;
		var entries = [];

		valueOutsideViewport = false;

		dataSeriesIndex = !isNullOrUndefined(dataSeriesIndex) && dataSeriesIndex >= 0 && dataSeriesIndex < this.chart.data.length ? dataSeriesIndex : 0;

		if (this.shared) {
			for (var i = 0; i < this.chart.data.length; i++) {
				dataSeries = this.chart.data[i];

				entry = dataSeries.getDataPointAtX(xValue, false);

				if (entry && entry.dataPoint && !isNullOrUndefined(entry.dataPoint.y) && dataSeries.visible) {
					entry.dataSeries = dataSeries;
					entries.push(entry);
				}
			}
		} else {
			dataSeries = this.chart.data[dataSeriesIndex];
			entry = dataSeries.getDataPointAtX(xValue, false);
		...
		if (entries.length > 0) {
			...
			for (var i = 0; i < entries.length; i++) {
				...
				if (entry.dataPoint.x < entry.dataSeries.axisX.viewportMinimum || entry.dataPoint.x > entry.dataSeries.axisX.viewportMaximum || entry.dataPoint.y < entry.dataSeries.axisY.viewportMinimum || entry.dataPoint.y > entry.dataSeries.axisY.viewportMaximum) {
					valueOutsideViewport = true;
				} else {
					valueOutsideViewport = false;
					break;
				}
			...
			if (toolTipInnerHtml !== null) {
				...
				var toolTipLeft, toolTipBottom;
				if (entries[0].dataSeries.type === "pie" || entries[0].dataSeries.type === "doughnut" || entries[0].dataSeries.type === "funnel" || entries[0].dataSeries.type === "pyramid") {
					toolTipLeft = mouseX - 10 - this.container.clientWidth;
				} else if (entries[0].dataSeries.type === "bar" || entries[0].dataSeries.type === "rangeBar" || entries[0].dataSeries.type === "stackedBar" || entries[0].dataSeries.type === "stackedBar100") {
					toolTipLeft = entries[0].dataSeries.axisY.convertValueToPixel(entries[0].dataPoint.y) - this.container.clientWidth << 0;
					toolTipLeft -= 10;
				} else {
					toolTipLeft = entries[0].dataSeries.axisX.convertValueToPixel(entries[0].dataPoint.x) - this.container.clientWidth << 0;
					toolTipLeft -= 10;
...
CanvasJSChart.version = "v3.4.9 GA";

if (window.CanvasJS) {
	//if (window.CanvasJS.StockChart) {
	//	if (window.console)
	//		window.console.log("StockChart already loaded");
	//}
	if (CanvasJSChart && !window.CanvasJS.Chart) {
		window.CanvasJS.Chart = CanvasJSChart;
	}
}
})();