...
function getSpecificSteamline1ConsumptionData() {
    ...
    $.ajax({
        ...
        url: "http://localhost:8090/EMSPro/steam/steamGraph",
    }).done(function (data) {
        console.log(data)
...
function getSpecificSteamConsumptionData() {
    ...
    $.ajax({
        ...
        url: "http://localhost:8090/EMSPro/steam/specificsteamconsumption",
    }).done(function (data) {
        console.log(data)
...
function steamDoughnut() {
    $.ajax({
        method: "GET",
        url: "http://localhost:8090/EMSPro/steam/steamBreakup",
    }).done(function (data) {
...
function loadDoughnutChart(data) {
    ...
    var chart = new CanvasJS.Chart("steam-doughnutChart", {
        ...
        backgroundColor: "#26293c",
        title: {
            text: data[0].steambreakup.toFixed(2),
        ...
        data: [{
            ...
            startAngle: 64,
            dataPoints: [
                { y: data[0].vhp, name: "VHP", indexLabel:  ((data[0].vhp / data[0].steambreakup) * 100).toFixed(2) + "%" },
                { y: data[0].hp, name: "HP", indexLabel: ((data[0].hp / data[0].steambreakup) * 100).toFixed(2) + "%" },
                { y: data[0].mp, name: "MP", indexLabel: ((data[0].mp / data[0].steambreakup) * 100).toFixed(2) + "%" },
                { y: data[0].lp, name: "LP", indexLabel: ((data[0].lp / data[0].steambreakup) * 100).toFixed(2) + "%" },
            ]
        }]
    });
    chart.render();
...
function steamtable() {
    $.ajax({
        url: "http://localhost:8090/EMSPro/steam/secSteam",
        method: "GET"
    }).done(function (data) {
        console.log(data)
        var table_data = '';
...
function steamtable2() {
    $.ajax({
        method: 'GET',
        url: 'http://localhost:8090/EMSPro/steam/steamTable'
    }).done(function (data) {
        console.log(data);
        document.getElementById("numact1").innerHTML = data.vhp;
        document.getElementById("numref1").innerHTML = data.vhp;

        document.getElementById("numact2").innerHTML = data.hp;
        document.getElementById("numref2").innerHTML = data.hp;

        document.getElementById("numact3").innerHTML = data.mp;
        document.getElementById("numref3").innerHTML = data.mp;

        document.getElementById("numact4").innerHTML = data.condensaterecovery;

        document.getElementById("numact5").innerHTML = data.specificwaterconsumption;
...
function steamDoughnutProgress2() {
    $.ajax({
        ...
        method: "GET",
        url: "http://localhost:8090/EMSPro/steam/steamCapacityUtilization",
    }).done(function (data) {
