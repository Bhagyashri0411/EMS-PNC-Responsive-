...
function getSpecificlinesteamGenData() {
    ...
    $.ajax({
        ...
        url: "http://localhost:8090/EMSPro/auth/Equipmentlevelopt/SteamGeneratorsGraph",
    }).done(function (data) {
        console.log(data)
...
function steamgeneratorstable() {
    $.ajax({
        ...
        method: 'GET',
        url: 'http://localhost:8090/EMSPro/auth/Equipmentlevelopt/SteamGenerators',
    }).done(function (data) {
        getsteamgeneratorstable(data);
    })
        .fail(function () {
            var failData = []

            getsteamgeneratorstable(failData)
        })
}
function getsteamgeneratorstable(data) {
     var max1=500;
    var table_data = '';
